...
// Definizione del client InfluxDB (Costruttore base senza Cloud Cert rigidi per locale IoT)
// SetInsecure permette la connessione a server locali Influx senza scontrarsi con SSL stretti o se Influx e HTTP puro
...
const unsigned long telemetryInterval = 10000; // Scrive su DB ogni 10 secondi (bilanciamento perfetto rete/peso)
...
void setupDatabase() {
    ...
    // I TimeSeries DB (InfluxDB) impazziscono se l'hardware non sa che ore sono (metterebbe anno 1970).
    // Questo è un passaggio fondamentale per i log a prova di manomissione.
...
void taskDatabase() {
    ...
    if (currentMillis - lastTelemetrySync >= telemetryInterval) {
        ...
        // Assegnazione Label Generica (per permettere query singole "Where Teca = A")
        telemetry.clearTags();
        ...
        // Conversione Stato Sistema in intero per grafici (0=Disarm, 1=Arm, 2=Alarm)
        int stateCode = 0;
        if (currentState == ARMED) stateCode = 1;
        if (currentState == ALARM_ACTIVE) stateCode = 2;
